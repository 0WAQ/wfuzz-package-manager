...
/// 上传包文件 
#[allow(dead_code)]
pub async fn upload_package(filepath: &str, metadata: &str) 
...
/// 获取包列表
#[allow(dead_code)]
pub async fn get_package_list() -> Result<Value, Box<dyn std::error::Error>> {
...
/// 下载包文件
#[allow(dead_code)]
pub async fn download_package(package_name: &str)
    -> Result<(), Box<dyn std::error::Error>>
{
    // 发送http请求并接收响应
